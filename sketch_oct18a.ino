...
void loop() {
  ...
  for (int i = 0; i < huskylens.count(); i++) {
    ...
    int positonY = map(abs(distanceFromCenterY), 0, screenCenterHeight, 0, 160);
    ...
    servoPos1 = servoPos1 + (targetPosX - servoPos1) / 10;  // move 1/5 of the difference each loop
    trackerServo.write(servoPos1);
    targetPosY = map(result.yCenter, 0, screenHeight, 45, 110); // adjust angles for your setup
    servoPos2 = servoPos2 + (targetPosY - servoPos2) / 10;
    tiltServo.write(servoPos2);
